...
router.post("/", function (req, res, next) {
    ...
    for (let i = 0; i < categorie.length; i++) {
        var query =
            "SELECT * from statistichecategorie where nome = '" +
            categorie[i] +
            "' limit 1";
        db.query(query)
            .then(function (result) {
                ...
                if (result.rowCount == 1) {
                    //esiste --> aggiorna tupla
                    q =
                        "UPDATE statistichecategorie SET rating = '" +
                        (result.rows[0].rating + 1) +
                        "' WHERE id = '" +
                        result.rows[0].id +
                        "'";
                } else {
                    //non esiste --> aggiungi tupla
                    q =
                        "insert into statistichecategorie (nome, rating) values ('" +
                        categorie[i] +
                        "', '1')";
                }
                db.query(q)
                    .then(function (rst) {
                        console.log(
                            "aggiornato correttamente statistichecategorie"
                        );
                    })
                    .catch(function (err) {
                        console.log(err.stack);
